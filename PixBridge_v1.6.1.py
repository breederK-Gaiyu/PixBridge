...
class PixBridge(QWidget):
    ...
    def import_images_with_copy(self, images):
        ...
        if not os.path.exists(type_folder):
            ...
            self.log(f"📁 创建本地文件夹: <code>{MEDIA_FOLDERS['image']}</code>")
        else:
            self.log(f"✅ 本地文件夹已存在: <code>{MEDIA_FOLDERS['image']}</code>")
    ...
    def import_media_direct(self, files, media_type):
        ...
        # 记录日志
        for f in files:
            self.log(f"🎬 检测到 {media_type}: <code>{f['name']}</code>")
            self.log(f"📍 原始路径: <span style='color:#888888;'>{f['path']}</span>")
        ...
        if success_count > 0:
            ...
            self.log(f"💡 <b>提示</b>: 文件未复制到本地，请勿移动或删除原文件")
